...
async def get_group_state(session: AsyncSession, chat_id: int) -> GroupState | None:
    result = await session.execute(select(GroupState).where(GroupState.chat_id == chat_id))
...
async def try_send_ad_for_chat(chat_id: int) -> bool:
    async with send_lock:
        async with SessionLocal() as session:
            ...
            if state is None:
                logger.warning("No DB row for chat_id=%s. Add it manually first.", chat_id)
                return False
            ...
            logger.info("Ad sent to chat_id=%s", chat_id)
            return True
